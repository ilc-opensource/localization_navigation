...
public:
	...
	int init();
	void release();
	bool getIntrinsics(int idx, Intrinsics &intrin);
...
bool RSWrapper::getIntrinsics(int idx, Intrinsics &intrin)
{
	return impl->getIntrinsics(idx, intrin);
}
...
bool RSWrapper::Impl::getIntrinsics(int idx, Intrinsics &intrin)
{
	...
	try {
		...
		intrin.fx = intrin.fx;
		intrin.fy = intrin.fy;
		intrin.ppx = intrin.ppx;
		intrin.ppy = intrin.ppy;
	} catch (const rs::error &e) {
		std::cerr << "librealsense error: " << e.get_failed_function() << ": " << e.what() << std::endl;
		return false;
	} catch (const std::exception &e) {
